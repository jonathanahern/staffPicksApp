...
if (url.includes("/pages/")) {
    ...
    if (eles.length > 0){
        ...
        fetch(`https://4bf672e66685.ngrok.io/api/pages?employeeid=${staffid}`, {
          method: "GET",
        })
          .then((res) => res.json())
          .then((resp) => {
      ...
      if (staffEle){ 
        fetch(`https://4bf672e66685.ngrok.io/api/pages/1/getStaff?shopDom=${shop}`, {
          method: "GET",
        })
          .then((res) => res.json())
          .then((resp) => {
...
if (url.includes('/products/') && pickedProducts && pickedProducts.includes(prodID)) {
  ...
  fetch(`https://4bf672e66685.ngrok.io/api/front_end/show?shop=${shop}&prodID=${meta.product.id}`, {
    method: "GET",
    })
  .then(res => res.json())
  .then(resp => {
...
function setPicks (shop) {
  ...
  if (secondDiff >= recheckTime){
    fetch(`https://4bf672e66685.ngrok.io/api/front_end?shop=${shop}`, {
        method: "GET",
      })
        .then((res) => res.json())
        .then((resp) => {
